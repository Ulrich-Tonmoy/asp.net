...
namespace UniversityCourseAndResultManagementSystem.Service
{
    public class ScheduleService : IScheduleService
    {
        ...
        {
            IQueryable<Schedule> schedules = _unitOfWork.ScheduleRepository.GetAllNoTrackingWithParam(scheduleParam, x => x.OrderBy(s => s.Id)).Where(x => x.IsDeleted == null || x.IsDeleted != true).Include(c => c.Course).Include(r => r.Room);
